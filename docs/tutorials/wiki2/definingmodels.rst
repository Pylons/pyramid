...
The first change we'll make to our stock ``pcreate``-generated application will be to define a wiki page :term:`domain model`.
...
The models code in our application will depend on a package which is not a dependency of the original "pyramidtut" application.  The original "pyramidtut" application was generated by the ``pcreate`` command; it doesn't know about our custom application requirements.

We need to add a dependency, the `bcrypt <https://pypi.python.org/pypi/bcrypt>`_ package, to our ``pyramidtut`` package's ``setup.py`` file by assigning this dependency to the ``requires`` parameter in the ``setup()`` function.
...
.. code-block:: text
    ...
    Successfully installed bcrypt-2.0.0 cffi-1.5.2 pycparser-2.14 pyramidtut-0.0


Remove ``mymodel.py``
---------------------

Let's delete the file ``pyramidtut/models/mymodel.py``. The ``MyModel`` class is only a sample and we're not going to use it.


Add ``user.py``
---------------

Create a new file ``pyramidtut/models/user.py`` with the following contents:

.. literalinclude:: src/models/pyramidtut/models/user.py
    :linenos:
    :language: py

This is a very basic model for a user who can authenticate with our wiki.

We discussed briefly in the previous chapter that our models will inherit from an SQLAlchemy :func:`sqlalchemy.ext.declarative.declarative_base`. This will attach the model to our schema.

As you can see, our ``User`` class has a class-level attribute ``__tablename__`` which equals the string ``users``. Our ``User`` class will also have class-level attributes named ``id``, ``name``, ``password_hash``, and ``role`` (all instances of :class:`sqlalchemy.schema.Column`). These will map to columns in the ``users`` table. The ``id`` attribute will be the primary key in the table. The ``name`` attribute will be a text column, each value of which needs to be unique within the column. The ``password_hash`` is a nullable text attribute that will contain a securely hashed password [1]_. Finally, the ``role`` text attribute will hold the role of the user.
...
Create a new file ``pyramidtut/models/page.py`` with the following contents:

.. literalinclude:: src/models/pyramidtut/models/page.py
    :linenos:
    :language: py
...
Open the ``pyramidtut/models/__init__.py`` file and edit it to look like the following:

.. literalinclude:: src/models/pyramidtut/models/__init__.py
    :linenos:
    :language: py
    :emphasize-lines: 8,9
...
We haven't looked at the details of this file yet, but within the ``scripts`` directory of your ``pyramidtut`` package is a file named ``initializedb.py``. Code in this file is executed whenever we run the ``initialize_pyramidtut_db`` command, as we did in the installation step of this tutorial.

.. note::

    The command is named ``initialize_pyramidtut_db`` because of the mapping defined in the ``[console_scripts]`` entry point of our project's ``setup.py`` file
...
Open ``pyramidtut/scripts/initializedb.py`` and edit it to look like the following:

.. literalinclude:: src/models/pyramidtut/scripts/initializedb.py
    :linenos:
    :language: python
...
Because our model has changed, and in order to reinitialize the database, we need to rerun the ``initialize_pyramidtut_db`` command to pick up the changes we've made to both the models.py file and to the initializedb.py file. See :ref:`initialize_db_wiki2` for instructions.
