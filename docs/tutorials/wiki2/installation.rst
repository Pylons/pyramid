...
.. code-block:: doscon
    ...
    c:\pyramidtut\> set VENV=c:\pyramidtut
...
.. code-block:: doscon
    ...
    c:\pyramidtut\> c:\Python27\Scripts\virtualenv %VENV%

Python 3.6:

.. code-block:: doscon

    c:\pyramidtut\> c:\Python36\Scripts\python -m venv %VENV%
...
.. code-block:: doscon
    ...
    c:\pyramidtut\> %VENV%\Scripts\pip install -e ".[testing]"
...
.. code-block:: bash
    ...
    pyramid-tm-1.1.1 pyramidtut pytest-3.0.5 pytest-cov-2.4.0 repoze.lru-0.6 \
...
You can run the ``pytest`` command to see test coverage information. This runs the tests in the same way that ``pytest`` does, but provides additional :term:`coverage` information, exposing which lines of your project are covered by the tests.
...
.. code-block:: bash
    ...
    rootdir: /Users/stevepiercy/pyramidtut, inifile:
    plugins: cov-2.4.0
    collected 2 items
    ...
    ------------------------------------------------------------------
    pyramidtut/__init__.py                   8      6    25%   7-12
    pyramidtut/models/__init__.py           22      0   100%
    pyramidtut/models/meta.py                5      0   100%
    pyramidtut/models/mymodel.py             8      0   100%
    pyramidtut/routes.py                     3      2    33%   2-3
    pyramidtut/scripts/__init__.py           0      0   100%
    pyramidtut/scripts/initializedb.py      26     16    38%   22-25, 29-45
    pyramidtut/views/__init__.py             0      0   100%
    pyramidtut/views/default.py             12      0   100%
    pyramidtut/views/notfound.py             4      2    50%   6-7
    ------------------------------------------------------------------
    TOTAL                                   88     26    70%
...
.. code-block:: bash
    ...
    $ $VENV/bin/pytest --cov=pyramidtut pyramidtut/tests.py -q
...
We need to use the ``initialize_pyramidtut_db`` :term:`console script` to initialize our database.

.. note::

    The ``initialize_pyramidtut_db`` command does not perform a migration, but rather it simply creates missing tables and adds some dummy data. If you already have a database, you should delete it before running ``initialize_pyramidtut_db`` again.

Type the following command, making sure you are still in the ``pyramidtut`` directory (the directory with a ``development.ini`` in it):
...
.. code-block:: bash
    ...
    $ $VENV/bin/initialize_pyramidtut_db development.ini
...
.. note::
    ...
    :app:`Pyramid` supports any persistent storage mechanism (e.g., object database or filesystem files). It also supports an additional mechanism to map URLs to code (:term:`traversal`). However, for the purposes of this tutorial, we'll only be using :term:`URL dispatch` and :term:`SQLAlchemy`.
